...
namespace Exercise6_DesignAWorkflowEngine
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Create Encode Video workflow
            var encodeVideo = new Workflow();

            // Add activities to Encode Video workflow
            encodeVideo.AddActivity(new UploadVideo());
            encodeVideo.AddActivity(new CallWebService());
            encodeVideo.AddActivity(new SendNotificationEmail());
            encodeVideo.AddActivity(new ChangeStatus());

            // Run workflow
            workflowEngine.Run(encodeVideo);
        }
    }
}
